use crate::{utils::{file_selector, read_dir, get_json}, class::{FileType, HubExtendedJson, HubNotice, AlphaCoverJson}};
...
pub fn valid(config:&Config)->Result<(),i32> {
    ...
    for service in config.mirror.services.clone() {
        ...
        if version_index>0 {
            let file_selector_res=file_selector(service.local.clone(), String::from(test_regex), version_index.try_into().unwrap());
            if let Err(e)= file_selector_res {
                errors_count+=1;
                println!("Error:Invalid service {} : can't find valid items in given local path : {}",service_name,e);
            }
    ...
    let extended_alpha_config: Result<AlphaCoverJson,String> = get_json(config.config.alpha_cover.clone());
    ...
    if let Err(e)=extended_alpha_config {
        errors_count+=1;
        println!("Error:Can't parse extended json config extended_alpha_config : {}",e);
    }
